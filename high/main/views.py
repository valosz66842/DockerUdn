            ...
            if product_list:
                for product in product_list:
                    ...
                    Limitedtime_modles = Limitedtime(seller=account,
                                                     ...
                                                     endtime=product[4],
                                                     id=make_id(),
                                                     srcset=str(model_to_dict(Product.objects.get(id=product[2])).get("srcset")))
            ...
            if request.method== "POST":
                ...
                try:
                    for product in product_list:
                        ...
                        business_list = Business(buyer="account",
                                                 seller="sell",
                                                 amount=product[1],
                        ...
                        temp_business.append(business_list)
                except:
                    atomicity=False
                    error_list.append(product_data.id+" error,")
                if atomicity:
                    for Order in temp_business:
                        Order.save()
                    result["status"] = True
                    request.session["ber_car_list"]=None
                else:
                    result["errcode"]=404
                    result["errmsg"]=error_list
