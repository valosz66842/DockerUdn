...
class UdnSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for article in article_each[:2]:#爬2篇文章
    ...
    def parse_article(self, response):#做每篇文章
        item = ScrapyudnItem()
        soup = BeautifulSoup(response.text)
        article_info =soup.select("div[class='shareBar__info']")[0].select("div[class='shareBar__info--author']")[0].text#記錄記者發文資訊
        a = soup.select("span")[7].findAll("p")#儲存內文
        content =''
        for n, i in enumerate(a[1::]):
            content+=i.text#去除空白儲存內文
        date=article_info.split(" ")[0]
        joindate="".join(article_info.split(" ")[0].split("-"))#紀錄日期
        Time="".join(article_info.split(date)[1][1:6].split(":"))#紀錄時間
        img = soup.select("img[class='lazyload']")[0].get("data-src")#紀錄發文圖片
