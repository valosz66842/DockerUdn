...
class kafka_connect:
    def __init__(self, ip_port):
        self.ip_port = "kafka:9092" # xx.xxx.xxx.xxx:9092 xxx自己kafka的IP
    ...
    def kafka_consumer(self):
        ...
        count = 0 # 紀錄筆數
        while True:
            records = consumer.consume(num_messages=500, timeout=1.0)  # 批次讀取 五百毫秒
            if len(records) > 0:
                count += 1
                for record in records:
                    ...
                    ip_port = "http://"+"elasticsearch"+":9200"
                    savedata_els(http_ip_port=ip_port,msgValue=msgValue)#傳送至els
...
def savedata_els(http_ip_port,msgValue):
    ...
    db = MySqlOperator("mysql", 'root', 'aaaa')  # 連線 user身分 密碼
    conn = db.conn
    df = pd.dataform( #紀錄使用者使用紀錄
        data=[{'title': msgValue["_source"]["UesrRecord"]["title"],
               "ip": msgValue["_source"]["ip"],
               "Date":msgValue["_source"]["Date"],
               "Time":msgValue["_source"]["Time"]
               }],
